...
import com.co.kr.mapper.BoardMapper;
...
public class BoardServiceImpl implements BoardService{
	...
	public int fileProcess(FileListVO fileListVO, MultipartHttpServletRequest request, HttpServletRequest httpReq) {
		...
		BoardContentDomain boardContentDomain = BoardContentDomain.builder()
				...
				if(fileListVO.getIsEdit() != null) {
					...
					// db 업데이트
					boardMapper.bdContentUpdate(boardContentDomain);
				}else {	
					// db 인서트
					boardMapper.bdcontentUpload(boardContentDomain);
					...
					if(!multipartFile.isEmpty()) {  // 파일 있을때 
						...
						try {
							...
							BoardFileDomain boardFileDomain = BoardFileDomain.builder()
								...
								// db 인서트
								boardMapper.bdfileUpload(boardFileDomain);
	...
	public void bdContentRemove(HashMap<String, Object> map) {
		boardMapper.bdContentRemove(map);
	}

	@Override
	public void bdFileRemove(BoardFileDomain studyFileDomain) {
		boardMapper.bdFileRemove(studyFileDomain);
	}
	
	// 하나만 가져오기
	@Override
	public BoardListDomain boardSelectOne(HashMap<String, Object> map) {
		return boardMapper.boardSelectOne(map);
	}

	// 하나 게시글 파일만 가져오기
	@Override
	public List<BoardFileDomain> boardSelectOneFile(HashMap<String, Object> map) {
		return boardMapper.boardSelectOneFile(map);
	}
	
	//검색창
	public List<BoardListDomain> boardSelectSelect(HashMap<String, Object> map) {
		return boardMapper.boardSelectSelect(map);
	};

	//전체 개수
		public int bdGetAll() {
			return boardMapper.bdGetAll();
		};
		
		public List<BoardListDomain> boardAllList(HashMap<String, Object> map){
			return boardMapper.boardAllList(map);
		};
}